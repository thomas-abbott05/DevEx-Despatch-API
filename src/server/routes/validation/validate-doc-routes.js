...
// POST /api/v1/validate-doc/:documentType
router.post('/:documentType', express.text({ type: ['text/xml', 'application/xml'] }), async (req, res) => {
  ...
  if (!VALID_DOCUMENT_TYPES.includes(documentType)) {
    return res.status(400).json({
      errors: [`${documentType} is not a valid option - must be one of 'order', 'receipt', 'despatch', 'order-cancel', 'order-change', 'fulfilment-cancel'.`],
